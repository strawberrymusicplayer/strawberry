...
void changeInstallName(const QString &bundlePath, const FrameworkInfo &framework, const QStringList &binaryPaths, bool useLoaderPath) {
  ...
  for (const QString &binary : binaryPaths) {
    ...
    if (useLoaderPath) {
      deployedInstallName = QLatin1String("@loader_path/") + QFileInfo(binary).absoluteDir().relativeFilePath(absBundlePath + QLatin1Char('/') + framework.binaryDestinationDirectory + QLatin1Char('/') + framework.binaryName);
    }
    else {
...
void deployRPaths(const QString &bundlePath, const QList<QString> &rpaths, const QString &binaryPath, bool useLoaderPath) {
  const QString absFrameworksPath = QFileInfo(bundlePath).absoluteFilePath() + QLatin1String("/Contents/Frameworks");
...
DeploymentInfo deployQtFrameworks(QList<FrameworkInfo> frameworks,
  ...
  while (frameworks.isEmpty() == false) {
    ...
    const QString deployedBinaryPath = framework.isDylib ? copyDylib(framework, bundlePath) : copyFramework(framework, bundlePath);
...
void deployPlugins(const ApplicationBundleInfo &appBundleInfo, const QString &pluginSourcePath,
  ...
  // GIO modules
  {
    ...
    const QStringList giomodules = QStringList() << "libgiognutls.so"
                                                 << "libgioopenssl.so";
...
QSet<QString> codesignBundle(const QString &identity,
  ...
  QProcess codesign;
  codesign.start("codesign", QStringList() << "--deep"
                                           << "-v" << appBundlePath);
  codesign.waitForFinished(-1);
  QByteArray err = codesign.readAllStandardError();
  if (codesign.exitCode() > 0) {
