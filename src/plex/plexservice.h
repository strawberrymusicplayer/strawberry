...
class PlexService : public StreamingService {
 ...
 public:
  ...
  QString server_machine_identifier() const { return server_machine_identifier_; }
 ...
 private:
  ...
  static ConnectionList OrderConnectionCandidates(const ConnectionList &connections, const bool owned);
  static bool HostLikelyReachableOnLan(const QString &host);
  void BeginConnectionSelection(const Server &server);
  void CancelConnectionSelection();
  ...
  QUrl server_url_;
  QString server_machine_identifier_;
  QString token_;
  ...
  qint64 last_update_;
  qint64 last_full_sync_;
  QStringList rejected_song_ids_;
