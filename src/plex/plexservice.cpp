...
constexpr qint64 kFullSyncIntervalSecs = 7LL * 24 * 60 * 60;  // Incremental syncs can't always detect deleted songs, so do a full sync regularly.
...
void PlexService::ReloadSettings() {
  ...
  const QString server_machine_identifier = s.value(PlexSettings::kServerMachineIdentifier).toString();
  ...
  last_full_sync_ = s.value(PlexSettings::kLastFullSync, 0).toLongLong();
  rejected_song_ids_ = s.value(PlexSettings::kRejectedSongIds).toStringList();
  ...
  // A different server was selected in the settings page, which only saves the server and leaves the URL empty, so select a reachable connection for it.
  const bool server_changed = server_machine_identifier != server_machine_identifier_;
  server_machine_identifier_ = server_machine_identifier;
  bool select_connection = server_changed && !server_machine_identifier_.isEmpty() && server_url_.isEmpty();

  // An unencrypted connection selected automatically for an owned server (no server token) would send the account token unencrypted, select a new connection.
  // Manually configured URLs don't have a machine identifier, and can still use HTTP.
  if (server_url_.scheme() == "http"_L1 && !server_machine_identifier_.isEmpty() && server_token_.isEmpty()) {
    qLog(Warning) << "Plex: Not using unencrypted connection" << server_url_.toString() << "for owned server, selecting a new connection.";
    server_url_.clear();
    select_connection = true;
  }

  if (select_connection && authenticated()) {
...
void PlexService::Deauthenticate() {
  ...
  // A running connection test would save the server token again when it finishes.
  CancelConnectionSelection();
...
void PlexService::HandleResourcesReply(QNetworkReply *reply) {
  ...
  // Logged out while the request was pending, don't select a server.
  if (token_.isEmpty()) return;
  ...
  for (const QJsonValue &value_device : array_devices) {
    ...
    server.connections = OrderConnectionCandidates(connections, owned);
    if (server.connections.isEmpty()) {
      qLog(Warning) << "Plex: Server" << name << "has no encrypted (HTTPS) connections, enter the server URL manually to use an unencrypted connection.";
    }
    else {
...
PlexService::ConnectionList PlexService::OrderConnectionCandidates(const ConnectionList &connections, const bool owned) {
  ...
  for (const Connection &connection : connections) {
    // Owned servers use the account token, never send it over an unencrypted connection, HTTP is only used when configured manually.
    const bool https = connection.uri.scheme() == "https"_L1;
    if (owned && !https) {
      qLog(Debug) << "Plex: Skipping unencrypted connection" << connection.uri.toString() << "for owned server.";
      continue;
    }
    ...
    // Prefer HTTPS over HTTP for connections that are equally likely to be reachable.
    score = score * 2 + (https ? 0 : 1);
...
void PlexService::CancelConnectionSelection() {
...
void PlexService::TryNextConnectionCandidate() {
  ...
  // The connection test sends the account token for owned servers, never over an unencrypted connection.
  if (connection_test_server_.owned && candidate.uri.scheme() != "https"_L1) {
    TryNextConnectionCandidate();
    return;
  }
...
void PlexService::HandleConnectionTestReply(QNetworkReply *reply, const QUrl &candidate_uri) {
  ...
  if (reply->error() == QNetworkReply::NoError && reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt() == 200) {
    ...
    const QJsonObject media_container = json_object["MediaContainer"_L1].toObject();
  ...
  if (!connection_test_server_.machine_identifier.isEmpty()) {
    server_machine_identifier_ = connection_test_server_.machine_identifier;
    s.setValue(PlexSettings::kServerMachineIdentifier, server_machine_identifier_);
  }
  s.endGroup();
...
void PlexService::GetSongs() {
  ...
  const bool full_sync = last_update_ <= 0 || QDateTime::currentSecsSinceEpoch() - last_full_sync_ >= kFullSyncIntervalSecs;
  ...
  songs_request_.reset(new PlexRequest(this, network_, url_handler_, existing_songs, rejected_song_ids_, full_sync ? 0 : last_update_), [](PlexRequest *request) { request->deleteLater(); });
...
void PlexService::DeleteSongs() {
  ...
  last_update_ = 0;
  last_full_sync_ = 0;
  rejected_song_ids_.clear();
  Settings s;
  s.beginGroup(PlexSettings::kSettingsGroup);
  s.remove(PlexSettings::kLastUpdate);
  s.remove(PlexSettings::kLastFullSync);
  s.remove(PlexSettings::kRejectedSongIds);
  s.endGroup();
...
void PlexService::SongsResultsReceived(const SongMap &songs, const QString &error) {
  ...
  if (error.isEmpty() && songs_request_) {
    ...
    if (!songs_request_->incremental()) {
      last_full_sync_ = QDateTime::currentSecsSinceEpoch();
      s.setValue(PlexSettings::kLastFullSync, last_full_sync_);
    }
    rejected_song_ids_ = songs_request_->rejected_song_ids();
    s.setValue(PlexSettings::kRejectedSongIds, rejected_song_ids_);
    s.endGroup();
  }
