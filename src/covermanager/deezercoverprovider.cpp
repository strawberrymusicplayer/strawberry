...
void DeezerCoverProvider::HandleSearchReply(QNetworkReply *reply, const int id) {
  ...
  for (const QJsonValue &json_value : std::as_const(array_data)) {
    ...
    if (json_object.contains("album"_L1) && json_object["album"_L1].isObject()) {  // Song search, so extract the album.
      object_album = json_object["album"_L1].toObject();
    }
    else {
      object_album = json_object;
    }

    if (!json_object.contains("id"_L1) || !object_album.contains("id"_L1)) {
      Error(u"Invalid Json reply, data array value object is missing ID."_s, json_object);
      continue;
    }
