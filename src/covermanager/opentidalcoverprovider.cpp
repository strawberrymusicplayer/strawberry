...
bool OpenTidalCoverProvider::StartSearch(const QString &artist, const QString &album, const QString &title, const int id) {
  ...
  qLog(Debug) << __PRETTY_FUNCTION__ << artist << album << title << id;

  if (artist.isEmpty() || album.isEmpty()) return false;

  if (!oauth_->authenticated() && !login_in_progress_ && QDateTime::currentSecsSinceEpoch() - last_login_attempt_.toSecsSinceEpoch() < 120) {
    return false;
  }
...
QByteArray OpenTidalCoverProvider::GetData(QNetworkReply *reply) {
  ...
  if (reply->error() != QNetworkReply::NoError && reply->error() < 200) {
    ...
    qLog(Error) << QStringLiteral("%1 (%2)").arg(reply->errorString()).arg(reply->error());
    return QByteArray();
  }

  const QByteArray data = reply->readAll();
  qLog(Debug) << __PRETTY_FUNCTION__ << reply->error() << http_status_code << data;
  if (!data.isEmpty()) {
    ...
    if (json_object.contains("errors"_L1) && json_object["errors"_L1].isArray()) {
      ...
      for (const QJsonValue &value : array_errors) {
        if (!value.isObject()) continue;
        const QJsonObject object_error = value.toObject();
        if (!object_error.contains("category"_L1) ||
            !object_error.contains("code"_L1) ||
            !object_error.contains("detail"_L1)) {
          continue;
        }
  ...
  if (reply->error() == QNetworkReply::NoError) {
    qLog(Error) << QStringLiteral("Received HTTP code %1").arg(reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt());
  }
  else {
    qLog(Error) << QStringLiteral("%1 (%2)").arg(reply->errorString()).arg(reply->error());
...
QJsonObject OpenTidalCoverProvider::GetJsonObject(const QByteArray &data) {
...
void OpenTidalCoverProvider::HandleSearchReply(QNetworkReply *reply, SearchRequestPtr search_request) {
  ...
  const QByteArray data = GetData(reply);
  if (data.isEmpty()) {
    if (login_in_progress_) {
  ...
  const QJsonObject json_object = GetJsonObject(data);
  ...
  int i = 0;
  for (const QJsonValue &value_included : array_included) {
    ...
    for (const QJsonValue &value_imagelink : array_imagelinks) {
