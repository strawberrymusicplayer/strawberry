...
void FileView::ReloadSettings() {
  ...
  tree_root_paths_ = file_settings.value(u"tree_root_paths"_s, QStringList()).toStringList();

  // If no paths configured, check for Music directory as sensible default
  if (tree_root_paths_.isEmpty()) {
    QString music_dir = QDir::homePath() + u"/Music"_s;
    if (QDir(music_dir).exists()) {
      tree_root_paths_ << music_dir;
    }
    // Otherwise leave empty - user will need to add roots manually
  }
...
void FileView::AddRootButtonClicked() {
  QString dir = QFileDialog::getExistingDirectory(
    this,
    tr("Select folder to add as tree root"),
    tree_root_paths_.isEmpty() ? QDir::homePath() : tree_root_paths_.first(),
...
void FileView::RemoveRootButtonClicked() {
  ...
  for (const QString &root : std::as_const(tree_root_paths_)) {
    if (path == root || path.startsWith(root + u'/')) {
