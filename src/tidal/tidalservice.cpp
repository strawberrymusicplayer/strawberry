...
QByteArray TidalService::AuthorizationHeader() const {

  if (oauth_->token_type().isEmpty() || oauth_->access_token().isEmpty()) {
    return QByteArray();
  }

  return oauth_->token_type().toUtf8() + " " + oauth_->access_token().toUtf8();
