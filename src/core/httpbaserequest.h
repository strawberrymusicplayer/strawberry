...
class HttpBaseRequest : public QObject {
 ...
 protected:
  ...
  virtual QString error() const { return error_; }
  virtual bool authenticated() const = 0;
  virtual bool use_authorization_header() const = 0;
  virtual QByteArray AuthorizationHeader() const = 0;

  virtual QNetworkReply *CreateGetRequest(const QUrl &url, const ParamList &params_provided = ParamList());
  ...
  virtual ReplyDataResult GetReplyData(QNetworkReply *reply, const bool call_error = true);
