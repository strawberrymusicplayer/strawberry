...
void Player::HandleLoadResult(const UrlHandler::LoadResult &result) {
  ...
  switch (result.type_) {
    ...
    case UrlHandler::LoadResult::TrackAvailable: {
      ...
      if (!song.samplerate() && result.samplerate_) {
        song.set_samplerate(result.samplerate_.value());
        update = true;
      }

      // If there was no bit depth info in song's metadata, use the one provided by URL handler, if there is one.
      if (!song.bitdepth() && result.bit_depth_) {
        song.set_bitdepth(result.bit_depth_.value());
        update = true;
      }

      // If there was no length info in song's metadata, use the one provided by URL handler, if there is one.
      if (!song.length_nanosec() && result.length_nanosec_) {
        song.set_length_nanosec(result.length_nanosec_.value());
        update = true;
      }
