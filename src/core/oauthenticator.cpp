...
void OAuthenticator::StartRefreshLoginTimer() {
  ...
  qLog(Debug) << settings_group_ << refresh_token_ << login_time_ << expires_in_;

  if (login_time_ > 0 && !refresh_token_.isEmpty() && expires_in_ > 0) {
    const qint64 time = std::max(5LL, expires_in_ - (QDateTime::currentSecsSinceEpoch() - login_time_));
    qLog(Debug) << time;
    if (time > 0) {
