...
void OAuthenticator::set_authorization_header(const bool authorization_header) {
...
void OAuthenticator::LoadSession() {
  ...
  token_type_ = s.value(kAccessToken).toString();
...
void OAuthenticator::StartRefreshLoginTimer() {
  ...
  if (!refresh_token_.isEmpty()) {
    const qint64 time = std::min(1LL, expires_in_ - (QDateTime::currentSecsSinceEpoch() - login_time_));
...
QByteArray OAuthenticator::AuthorizationHeader() const {
...
void OAuthenticator::Authenticate() {
  ...
  qLog(Debug) << "OAuthenticator" << url.toDisplayString(QUrl::FullyDecoded);
...
void OAuthenticator::Deauthenticate() {
...
QNetworkReply *OAuthenticator::CreateRequest(const ParamList &params) {
  ...
  QUrl new_url(access_token_url_);
  ...
  if (authorization_header_ && !client_id_.isEmpty() && !client_secret_.isEmpty()) {
    const QString authorization_header = client_id_ + u':' + client_secret_;
    request.setRawHeader("Authorization", "Basic " + authorization_header.toUtf8().toBase64());
  }
...
void OAuthenticator::RequestAccessToken(const QString &code, const QUrl &redirect_url) {
  ...
  ParamList params = ParamList() << Param(u"grant_type"_s, u"authorization_code"_s)
                                 << Param(u"code"_s, code)
                                 << Param(u"code_verifier"_s, code_verifier_)
                                 << Param(u"redirect_uri"_s, redirect_url.toString());
...
void OAuthenticator::RenewAccessToken() {
  ...
  ParamList params = ParamList() << Param(u"client_id"_s, client_id_)
                                 << Param(u"grant_type"_s, u"refresh_token"_s)
...
void OAuthenticator::AccessTokenRequestFinished(QNetworkReply *reply) {
  ...
  if (reply->error() != QNetworkReply::NoError || reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt() != 200) {
    ...
    QJsonDocument json_document = QJsonDocument::fromJson(data, &json_error);
    if (json_error.error == QJsonParseError::NoError && !json_document.isEmpty() && json_document.isObject()) {
      const QJsonObject json_object = json_document.object();
      if (!json_object.isEmpty() && json_object.contains("error"_L1) && json_object.contains("error_description"_L1)) {
