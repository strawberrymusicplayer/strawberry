...
JsonBaseRequest::JsonObjectResult JsonBaseRequest::ParseJsonObject(const QByteArray &data) {
...
JsonBaseRequest::JsonObjectResult JsonBaseRequest::ParseJsonObject(QNetworkReply *reply) {

  const ReplyDataResult reply_data_result = GetReplyData(reply);
  if (!reply_data_result.success()) {
    return JsonObjectResult(reply_data_result);
  }

  return ParseJsonObject(reply_data_result.data);
...
JsonBaseRequest::JsonArrayResult JsonBaseRequest::GetJsonArray(const QJsonObject &json_object, const QString &name) {
  ...
  if (json_value_result.json_value.isArray()) {
    return JsonArrayResult(ErrorCode::ParseError, QStringLiteral("Json object is value %1 is not a array.").arg(name));
  }
