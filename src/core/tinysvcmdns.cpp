...
void TinySVCMDNS::CreateMdnsd(const uint32_t ipv4, const QString &ipv6) {
  ...
  // Set our hostname
  mdnsd_set_hostname(mdnsd, QString(host + ".local").toUtf8().constData(), ipv4);
...
void TinySVCMDNS::PublishInternal(const QString &domain, const QString &type, const QByteArray &name, quint16 port) {

  // Some pointless text, so tinymDNS publishes the service correctly.
  const char *txt[] = { "cat=nyan", nullptr };

  for (mdnsd *mdnsd : mdnsd_) {
    mdnsd_register_svc(mdnsd, name.constData(), QString(type + ".local").toUtf8().constData(), port, nullptr, txt);
  }
