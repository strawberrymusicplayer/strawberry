...
GeniusLyricsProvider::GeniusLyricsProvider(const SharedPtr<NetworkAccessManager> network, QObject *parent)
  ...
  oauth_->set_scope(u"me"_s);
  oauth_->set_random_port(false);
  oauth_->set_authorization_header(false);
...
QString GeniusLyricsProvider::token_type() const {

  return oauth_->token_type();

}

QString GeniusLyricsProvider::access_token() const {

  return oauth_->access_token();
...
void GeniusLyricsProvider::Deauthenticate() {
  ...
  oauth_->Deauthenticate();

}

QByteArray GeniusLyricsProvider::AuthorizationHeader() const {

  return oauth_->AuthorizationHeader();
...
void GeniusLyricsProvider::HandleSearchReply(QNetworkReply *reply, const int id) {
  ...
  for (const QJsonValue &value_hit : array_hits) {
    ...
    QNetworkReply *new_reply = network_->get(request);
    replies_ << new_reply;
