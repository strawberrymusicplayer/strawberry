...
ScrobblingAPI20::ReplyDataResult ScrobblingAPI20::GetReplyData(QNetworkReply *reply, const bool call_error) {

  const ReplyDataResult result = HttpBaseRequest::GetReplyData(reply, call_error);
  if (result.error_code != ErrorCode::Success) {
    const ScrobbleErrorCode lastfm_error_code = static_cast<ScrobbleErrorCode>(result.api_error);
...
void ScrobblingAPI20::RedirectArrived() {
  ...
  if (local_redirect_server_->error().isEmpty()) {
    QUrl url = local_redirect_server_->request_url();
    if (url.isValid()) {
...
void ScrobblingAPI20::AuthenticateReplyFinished(QNetworkReply *reply) {
  ...
  const JsonObjectResult json_object_result = GetJsonObject(reply);
...
void ScrobblingAPI20::ScrobbleRequestFinished(QNetworkReply *reply, ScrobblerCacheItemPtrList cache_items) {
  ...
  const QJsonValue value_scrobble = json_object["scrobble"_L1];
...
void ScrobblingAPI20::SingleScrobbleRequestFinished(QNetworkReply *reply, ScrobblerCacheItemPtr cache_item) {
  ...
  const QJsonValue value_scrobble = object_attr["scrobble"_L1];
  if (!value_scrobble.isObject()) {
