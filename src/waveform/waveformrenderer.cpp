...
namespace {
// Full-scale value of the stored int8 amplitudes; the body holds qint8 in
// [-127, 127] so 127 maps to unity before the curve is applied.
...
QPixmap WaveformRenderer::RenderToPixmap(const QByteArray &data, const QSize size, const QPalette &palette, const QColor &bar_color) {
  ...
  float peak = 0.0f;
  stream >> peak;
  // The header peak is consumed to advance the stream to the column body. It is
  // not used to normalize: bars are scaled against the fixed int8 full scale so
  // genuinely quiet tracks render proportionally short (the contrast Bug 2 wants).
  Q_UNUSED(peak)
  ...
  for (int x = 0; x < W; ++x) {
    ...
    // Reduce the base envelope to this column via min/max, never averaging, so
    // transients survive. qsizetype arithmetic avoids index overflow.
