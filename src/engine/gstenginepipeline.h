...
class GstEnginePipeline : public QObject {
 ...
 public:
  ...
  void SetStateDelayedAsync(const GstState state, const qint64 delay_ms);
  Q_INVOKABLE QFuture<GstStateChangeReturn> Play(const quint64 offset_nanosec);
 ...
 private slots:
  ...
  void SetStateDelayed(const GstState state, const qint64 delay_ms);
  void SetStateDelayedSlot();
