...
Player::Player(SharedPtr<TaskManager> task_manager, SharedPtr<UrlHandlers> url_handlers, SharedPtr<PlaylistManager> playlist_manager, QObject *parent)
...
void Player::UrlHandlerRegistered(UrlHandler *url_handler) {
...
void Player::SavePlaybackStatus() {
  ...
  s.beginGroup(kSettingsGroup);
  s.setValue("playback_state", static_cast<int>(GetState()));
  if (GetState() == EngineBase::State::Playing || GetState() == EngineBase::State::Paused) {
    s.setValue("playback_playlist", playlist_manager_->active()->id());
    s.setValue("playback_position", engine_->position_nanosec() / kNsecPerSec);
  }
  else {
