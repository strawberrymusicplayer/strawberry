...
DropboxSongsRequest::~DropboxSongsRequest() {
...
void DropboxSongsRequest::GetFolderListFinished(QNetworkReply *reply) {
  ...
  for (const QJsonValue &value_entry : entires) {
    ...
    if (tag == "deleted"_L1) {
      ...
      collection_backend_->DeleteSongsByUrlAsync(url);
      continue;
    }
    ...
    if (service_->IsSupportedFiletype(path)) {
...
void DropboxSongsRequest::Error(const QString &error_message, const QVariant &debug_output) {
  ...
  DropboxBaseRequest::Error(error_message, debug_output);

  Q_EMIT ShowErrorDialog(error_);
