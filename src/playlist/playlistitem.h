...
class PlaylistItem : public std::enable_shared_from_this<PlaylistItem> {
 public:
  ...
  quint64 effective_beginning_nanosec() const {
    if (HasTemporaryMetadata() && temp_metadata_.is_valid() && temp_metadata_.beginning_nanosec()) {
      return temp_metadata_.beginning_nanosec().value();
    }
    else {
      return Metadata().beginning_nanosec().value();
    }
  }
  quint64 effective_end_nanosec() const {
    if (HasTemporaryMetadata() && temp_metadata_.is_valid() && temp_metadata_.end_nanosec()) {
      return temp_metadata_.end_nanosec().value();
    }
    else {
      return Metadata().end_nanosec().value();
    }
  }
