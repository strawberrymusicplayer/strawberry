...
// Decodes the bundled WAV through the real GStreamer pipeline on a dedicated
// worker thread (the pipeline asserts it does not run on the qApp thread) and
// confirms a versioned magic blob is produced on EOS.
TEST(WaveformPipelineTest, DecodesWavToMagicBlob) {
  ...
  // Start() asserts QThread::currentThread() != qApp->thread(), so drive it from
  // a dedicated worker thread, mirroring MoodbarLoader.
