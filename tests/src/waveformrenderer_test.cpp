...
// Builds a well-formed SWVF blob with count buckets, each holding mn/mx, and a
// caller-supplied header peak. Mirrors the layout WaveformBuilder writes so the
// renderer's IsValidBlob gate accepts it.
...
TEST(WaveformRendererTest, SilentTrackPeakZeroDoesNotCrash) {
  ...
  // A structurally valid blob with a zero header peak reaches the renderer's
  // peak parse; it must not divide by zero (peak is consumed, never used to
  // normalize). A flat zero envelope simply produces no bars.
...
TEST(WaveformRendererTest, DifferentBarColorsProduceDifferentPixmaps) {
  ...
  // The bar color is caller-supplied; rendering the same blob in two colors must
  // yield two visibly different images (the proxy relies on this to composite a
  // played pixmap and an unplayed pixmap around the live split).
...
TEST(WaveformRendererTest, GentleCurvePreservesLoudQuietContrast) {
  ...
  // The 0.65 power-law must keep a loud track's bars taller than a quiet track's
  // (it lifts quiet passages without saturating everything to full height).
  ...
  // Count painted (non-background) pixels in a column; a louder track paints
  // more of the column height than a quieter one.
