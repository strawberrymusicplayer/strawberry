...
class TestID3v2 : public CppUnit::TestFixture {
 ...
 public:
  ...
  void testDowngradeUTF8ForID3v23_1() {
    ...
    ID3v2::TextIdentificationFrame *f = new ID3v2::TextIdentificationFrame(ByteVector("TPE1"), String::UTF8);
    StringList sl;
    sl.append("Foo");
    f->setText(sl);
  ...
  void testDowngradeUTF8ForID3v23_2() {
    ...
    ByteVector data = f->render();
    CPPUNIT_ASSERT_EQUAL(static_cast<size_t>(4 + 4 + 2 + 1 + 3 + 2 + 2 + 6 + 2), data.size());
  ...
  void testUTF16BEDelimiter() {
    ...
    sl.append("Bar");
    f.setText(sl);
    CPPUNIT_ASSERT_EQUAL(static_cast<size_t>(4 + 4 + 2 + 1 + 6 + 2 + 6), f.render().size());
  }
  ...
  void testUTF16Delimiter() {
    ...
    sl.append("Bar");
    f.setText(sl);
    CPPUNIT_ASSERT_EQUAL(static_cast<size_t>(4 + 4 + 2 + 1 + 8 + 2 + 8), f.render().size());
  }
  ...
  void testParseAPICv22() {
    ...
    ID3v2::AttachedPictureFrame *frame = dynamic_cast<Strawberry_TagLib::TagLib::ID3v2::AttachedPictureFrame *>(factory->createFrame(data, &header));
  ...
  void testDontRender22() {
    ...
    ID3v2::UnknownFrame *frame =
      dynamic_cast<Strawberry_TagLib::TagLib::ID3v2::UnknownFrame *>(factory->createFrame(data, &header));
  ...
  void testParseSynchronizedLyricsFrame() {
    ...
    CPPUNIT_ASSERT_EQUAL(static_cast<size_t>(2), stl.size());
  ...
  void testUpdateGenre23_1() {
    ...
    ID3v2::TextIdentificationFrame *frame =
      dynamic_cast<Strawberry_TagLib::TagLib::ID3v2::TextIdentificationFrame *>(factory->createFrame(data, &header));
  ...
  void testCompressedFrameWithBrokenLength() {
    ...
    if (zlib::isAvailable()) {
      ID3v2::AttachedPictureFrame *frame = dynamic_cast<Strawberry_TagLib::TagLib::ID3v2::AttachedPictureFrame *>(f.ID3v2Tag()->frameListMap()["APIC"].front());
      CPPUNIT_ASSERT(frame);
      ...
      ID3v2::UnknownFrame *frame = dynamic_cast<Strawberry_TagLib::TagLib::ID3v2::UnknownFrame *>(f.ID3v2Tag()->frameListMap()["APIC"].front());
      CPPUNIT_ASSERT(frame);
    }
  }
  ...
  void testW000() {
    ...
    ID3v2::UrlLinkFrame *frame =
      dynamic_cast<Strawberry_TagLib::TagLib::ID3v2::UrlLinkFrame *>(f.ID3v2Tag()->frameListMap()["W000"].front());
    CPPUNIT_ASSERT(frame);
  ...
  void testDeleteFrame() {
...

    {
      ...
      CPPUNIT_ASSERT_EQUAL(static_cast<size_t>(1), t->frameList("TCON").size());
